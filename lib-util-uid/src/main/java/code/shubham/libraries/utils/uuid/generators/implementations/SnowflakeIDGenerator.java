...
import code.shubham.libraries.utils.uuid.generators.UniqueSequenceIdGenerator;

import java.time.Instant;

import static code.shubham.libraries.utils.uuid.constants.Constants.NODE_ID_BIT_LENGTH;
import static code.shubham.libraries.utils.uuid.constants.Constants.SEQUENCE_BIT_LENGTH;

public class SnowflakeIDGenerator implements UniqueSequenceIdGenerator {
    ...
    private void isNodeIdValidOrThrowException(final long nodeId) {
        if (nodeId > this.maxNodeIdValue || nodeId < 0)
            throw new IllegalArgumentException(String.format("Node Id can't be greater than %d or less than 0", this.maxNodeIdValue));
    }
    ...
    public Long generate() {
        synchronized (this) {
            ...
            if (currentTimeStamp < this.lastTimestamp)
                throw new RuntimeException(String.format("Clock moved backwards. Refusing to generate id for %s milliseconds", (this.lastTimestamp - currentTimeStamp)));
            ...
            return ((currentTimeStamp - EPOCH_START) << timeStampLeftSift)
                        | (this.nodeId << SEQUENCE_BIT_LENGTH)
